...
export default function HowItWorks() {
  return (
    <section className="min-h-full flex items-center bg-primary-dark pt-14 md:pt-0">
      <div className="max-w-5xl mx-auto px-6 lg:px-8 py-12 lg:py-20 w-full text-center">
        <p className="text-primary-light font-semibold text-sm md:text-base tracking-widest uppercase">Getting Started</p>
          ...
          {steps.map((step) => (
            <div key={step.step} className="text-center">
              <div className="w-20 h-20 mx-auto bg-primary rounded-full flex items-center justify-center relative">
                ...
                <span className="absolute -top-1.5 -right-1.5 w-8 h-8 bg-accent text-white text-sm font-bold rounded-full flex items-center justify-center">
                  {step.step}
                </span>
              </div>
              <h3 className="mt-5 text-xl font-bold text-white">{step.title}</h3>
              <p className="mt-2 text-white/60 text-base leading-relaxed max-w-[220px] mx-auto">
                {step.desc}
              </p>
            </div>
          ))}
        ...
        <p className="mt-12 text-white/50 text-base">
